...
async function getBot(server) {
    ...
    for (let i = 0; i < botvang.length; i++) {
        ...
        html += `<div class="list1"><b style="color:green;">[Online]</b> Vũ trụ ` + botvang[i].Server + `s: <b style="color:blue;">` + botvang[i].Name + `</b> Khu <span style="color:red;">` + botvang[i].Zone + `</span><span> Số vàng <span style="color:red;">` + numberWithCommas(botvang[i].Gold) + `<span></span></div>`
    }
    return html
}
